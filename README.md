...
- 🔐 **登录认证** —— 手机号 + 密码登录，Token 本地缓存
- 📁 **文件夹管理** —— 创建、列表、删除、移动
- 📄 **文档管理** —— 创建、获取、保存、删除
- 📋 **大纲导出** —— 导出为 Markdown 格式

### 版本亮点（v1.0.0）

| 里程碑 | 能力 | 说明 |
| :--- | :--- | :--- |
| **M1 · P0** | 🔄 Markdown 双向往返 | 大纲 ↔ 幕布，含 `note` 备注、勾选 `[x]` 往返 |
| **M1 · P0** | 🔁 自动重登 | Token 过期自动重新登录，401 仅重试 1 次防死循环 |
| **M2 · P1** | 🛡️ 网络健壮性 | `timeout=15s` + 5xx/超时指数退避重试 2 次 |
| **M2 · P1** | 🔍 本地递归搜索 | `search` 按名称匹配、大小写不敏感，递归遍历全部子文件夹 |
| **M2 · P1** | 🔒 安全加固 | `.env.mubu` 加载（环境变量优先于文件）、Token 文件权限 `0o600` |
| **M3 · P2** | 🧪 工程化 | 全量类型注解、`requirements.txt`、CI 自动化测试 |
